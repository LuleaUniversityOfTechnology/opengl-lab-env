...
namespace Example {
	...
	*/
	...
	GLfloat* ExampleApp::createTriangle(GLfloat a, GLfloat cx, GLfloat cy) {
		float r = (a * sqrt(3))/6;
		float h = (a * sqrt(3))/2;
	...
	void ExampleApp::translatePoint(GLfloat* px, GLfloat* py, GLfloat tx, GLfloat ty) {
		*px += tx;
		*py += ty;
	}

	void ExampleApp::rotatePoint(GLfloat* px, GLfloat* py, float angle) {
		GLfloat x = *px;
		GLfloat y = *py;
	...
	GLfloat* ExampleApp::calcTriangle(GLfloat ps[4]) {
		float dx = ps[2] - ps[0];
		float dy = ps[3] - ps[1];
	...
	GLfloat* ExampleApp::calcSnowflake(GLfloat* points, int num_points, int depth) {
